...
/**
 * {@link LlamaEntityWriter} クラスを検証するテストクラスです.
 */
class LlamaEntityWriterTest {
  ...
  /**
   * {@link LlamaEntityWriter#writeHorse(EggDataBase)} のためのテスト・メソッド。
   */
  @Test
  final void testWriteHorse() {
    ...
    try (MockedStatic<Bukkit> bukkit = mockStatic(Bukkit.class)) {
      ...
      boolean isWriteSuccess = llamaEw.writeHorse(eggData);
      assertTrue(isWriteSuccess);

      when(eggData.getArmor()).thenReturn((String) llamaEggDataMap.get(dataKeyArmor));
      isWriteSuccess = llamaEw.writeHorse(eggData);
      assertTrue(isWriteSuccess);
    }
  }
}
